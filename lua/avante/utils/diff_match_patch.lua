...
local diff_main, diff_cleanupSemantic, diff_cleanupEfficiency, diff_levenshtein, diff_prettyHtml

local match_main

local patch_make, patch_toText, patch_fromText, patch_apply
...
local _diff_compute, _diff_bisect, _diff_halfMatchI, _diff_halfMatch, _diff_cleanupSemanticScore, _diff_cleanupSemanticLossless, _diff_cleanupMerge, _diff_commonPrefix, _diff_commonSuffix, _diff_commonOverlap, _diff_xIndex, _diff_text1, _diff_text2, _diff_toDelta, _diff_fromDelta
...
function _diff_fromDelta(text1, delta)
  ...
  for token in gmatch(delta, "[^\t]+") do
    ...
    if tokenchar == "+" then
      ...
      local decoded = gsub(param, "%%(.?.?)", function(c)
...
local _patch_addContext, _patch_deepCopy, _patch_addPadding, _patch_splitMax, _patch_appendText, _new_patch_obj
...
function patch_fromText(textline)
  ...
  while textPointer <= #text do
    ...
    while true do
      ...
      local invalidDecode = false
      local decoded = gsub(line, "%%(.?.?)", function(c)
        local n = tonumber(c, 16)
        if (#c ~= 2) or (n == nil) then
          invalidDecode = true
          return ""
